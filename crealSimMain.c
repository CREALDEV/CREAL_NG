...
#include "include/libcudacreal.h"

#include "include/struct_definitions.h" //struct definitions
...
int main(int argc, char **argv)
{
					...
					if (!argv[2])
					{
						quitWithError("An output file was not entered\n"); //shows application usage	
					...
					outputFP = fopen(argv[2],"w"); //open the file descriptor
					
					if (outputFP < 0)
					{
						perror("Something happening trying to open output file descriptor\n"); //error
					...
					fputs(CREAL_HEADERS,outputFP); //these are the headers of creal the next generation
					fputs(ETC_BEGINNING_DEFINITIONS,outputFP); //these are the other headers needed
					printOutMaxPreprocessedValues(55, 55, 55, 55, &outputFP);
					fputs(CREAL_T_TYPE,outputFP); //these are the headers of creal the next generation
					
					fputs(ACTION_FUNCTION_POINTER,outputFP); //the function pointer declaration
					fputs(PRE_ACTION_STRUCT,outputFP); //action 
					fputs(PRE_CONDITIONLIST_STRUCT,outputFP); //conditions
					
					
					
					fputs(PRE_ACTION_LIST,outputFP); //the action list
					
					printOutStaticActionList(10, 10,  &outputFP ); //this is used to print out the contents of the 
					
					
					printOutStaticCrealNodes(1000,1000,&outputFP); //this prints out the static creal node data
					
					fputs(CREAL_CSV_STRUCTS,outputFP);

					printOutNodeActionAndPropArrays(1000,1000,1000,&outputFP); //print out the structures
					
					fputs(MAIN_CREAL_PROCESSING_FUNCTIONS,outputFP);
					
					writeOutActionConditionFunctionsPrototypes(&root, &outputFP); //these are the function prototypes
					
					fputs(MAIN_PART,outputFP); //beginning of the 
					...
					fputs(MAIN_GAME_LOOP,outputFP);
					
					fputs(END_PART,outputFP);
					
					writeOutActionConditionFunctions(&root, &outputFP); //this is where we write out the functions in relation to the 
					
					printOutFromCTF("code_templates/process_functions.ctf",&outputFP);
					
					fputs("void crealInit(void)\n{",outputFP);
					abstractJsonData(&root, &outputFP); //this is the main function of abstracting json data
					fputs("}",outputFP);
			...
			fclose(outputFP); //make sure to close the file pointer/descriptor
